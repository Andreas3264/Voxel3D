	...
	public void placeStructure(int x, int y, int z, GenerationContext context)
	{
		for(int xx = -2; xx <= 2; xx++)
		{
			for(int yy = 3; yy <= 5; yy++)
			{
				...
				{
					context.placeBlock(x + xx, y + yy, z + zz, BirchLeavesBlock.getInstance());
				}
			}
		}
		...
		for(int h = 0; h < 5; h++)
		{
			context.placeBlock(x, y + h, z, BirchLogBlock.getInstance());
		}
		...
		for(int ox = -1; ox <=1 ; ox++) {
			for(int oy = -1; oy <= 1; oy++) {
				for(int oz = -1; oz <= 1; oz++) {
					...
					random.setSeed(rx * 2133 + ry * 2142767 + rz * 736329);
					
					int tests = (Settings.CHUNK_SIZE3) / 256;
