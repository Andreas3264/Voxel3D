...
public class CrimsonTree implements Structure {
	...
	{	
			...
			for(int yy = -3; yy <= 3; yy++)
			{
				for(int zz = -9; zz <= 9; zz++)
				{
					if(xx*xx + yy*yy*8 + zz*zz < 9*9)
						context.placeBlock(x + xx, y + 10 + yy, z + zz, CrimsonLeavesBlock.getInstance());
				}
			}
		}
		...
		for(int h = 0; h < 12; h++)
		{
			context.placeBlock(x, y + h, z, CrimsonLogBlock.getInstance());
		}
			...
			for(int oy = -1; oy <= 1; oy++) {
				for(int oz = -1; oz <= 1; oz++) {
					...
					for(int i = 0; i < tests; i++)
					{
						...
						if(context.generator.getBlock(x, y - 1, z) instanceof CrimsonGrassBlock)
						{
